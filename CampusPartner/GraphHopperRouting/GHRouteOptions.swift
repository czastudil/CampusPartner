...
open class RouteOptions: NSObject {
    ...
    // ADDED BY CINDY ZASTUDIL
    
    /**
     Sets the ch.disable variable to determine whether or not to avoid certain road classes and environments.
    */
    public var disable: Bool?
    
    /**
     The specified road classes and environments to avoid.
    */
    ...
    internal var params: [URLQueryItem] {
        ...
        // NEXT TWO PARAMS ADDED BY CINDY ZASTUDIL
        
        if let disable = self.disable {
            params.append(URLQueryItem(name: "ch.disable", value: String(disable)))
        }
        
        if let avoid = self.avoid {
            params.append(URLQueryItem(name: "avoid", value: avoid))
        }
